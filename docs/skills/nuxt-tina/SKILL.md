...
- `useTina(props)`: pass the full server response ref (not just unwrapped
  ...
  isInIframe, quickEditEnabled, bridgeError, refresh }`. Patches over
  `postMessage`, no refetch.
...
- `useTinaBridge()`: low-level `postMessage` access (`isInIframe`,
  `sendToAdmin`, `onAdminMessage`, `bridgeError`). Most code should use
  `useTina()` instead.
...
is `'tina'` or `'cloudinary'`, default `'tina'`), `enabled`, `debug` (default
`false`: logs bridge connections and dropped messages, dev-only).
